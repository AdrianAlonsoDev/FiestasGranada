...
public class Boton extends FrameLayout implements View.OnClickListener {
     ...
     * @param attrs    attributes of the method
     * @param defStyle style int defined in the method
     */
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        int circleStartColor = array.getColor(R.styleable.LikeButton_circle_start_color, 0);

        if (circleStartColor != 0)
            circleView.setStartColor(circleStartColor);

        int circleEndColor = array.getColor(R.styleable.LikeButton_circle_end_color, 0);
            ...
            animatorSet.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    if (animationEndListener != null) {
                        animationEndListener.onAnimationEnd(Boton.this);
                    }
                }
            });
     ...
     *
     * @param resId int Id where it is residing
     */
    public void setLikeDrawableRes(@DrawableRes int resId) {
        likeDrawable = ContextCompat.getDrawable(getContext(), resId);

        if (iconSize != 0) {
            likeDrawable = BotonUtils.resizeDrawable(getContext(), likeDrawable, iconSize, iconSize);
        }
     ...
     *
     * @param likeDrawable the drawable resource.
     */
    public void setLikeDrawable(Drawable likeDrawable) {
        this.likeDrawable = likeDrawable;

        if (iconSize != 0) {
            this.likeDrawable = BotonUtils.resizeDrawable(getContext(), likeDrawable, iconSize, iconSize);
        }
    ...
    public void setUnlikeDrawableRes(@DrawableRes int resId) {
        ...
        if (iconSize != 0) {
            unLikeDrawable = BotonUtils.resizeDrawable(getContext(), unLikeDrawable, iconSize, iconSize);
        }
     ...
     * This drawable will be shown when the button is in on unLiked state.
     *
     * @param unLikeDrawable the drawable resource.
     */
    public void setUnlikeDrawable(Drawable unLikeDrawable) {
        this.unLikeDrawable = unLikeDrawable;

        if (iconSize != 0) {
            this.unLikeDrawable = BotonUtils.resizeDrawable(getContext(), unLikeDrawable, iconSize, iconSize);
        }
     ...
     *
     * @param currentIconType the current icon type.
     */
     ...
     *
     * @param iconSize the icon size Dp
     */

    public void setIconSizeDp(int iconSize) {
        setIconSizePx((int) BotonUtils.dipToPixels(getContext(), (float) iconSize));
    }
     ...
     * the like effect are also updated to reflect the size of the icon.
     *
     * @param iconSize the icon size Px
     */
    ...
    private Icon parseIconType(String iconType) {
        List<Icon> icons = BotonUtils.getIcons();
     ...
     *
     * @param likeListener assign OnLike listener.
     */
     ...
     *
     * @param animationEndListener Assign anim. end listener.
     */
     ...
     * or unliked.
     *
     * @param status Status if it is liked or not.
     */
